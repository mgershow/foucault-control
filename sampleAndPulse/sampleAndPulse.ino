...
bool state;
bool transition;
int timeToActivate = 100;
int durToActivate = 10;
bool dirToActivate = true;
...
void adc0_isr(void) {
    bool oldstate = state;
    int t = thresh + state ? hysteresis : -1*hysteresis;
    state = adc->adc0->analogReadContinuous() > t;
    if (state^oldstate && !(state ^ dirToActivate)) {
      transition = true;
    }
